...
def flood(img,mask,seed_list,component_dots):
    """
    The flodd use the layer prior travel.

    check every  neighbor of the seed in the seed list. 
    gernerate new seed _list
    input: 
    output: a list of (y,x)  and set the mask
    """
    if not seed_list:
        return
    ...
    flood(img,mask,next_surround_seed_list,mean_value):
