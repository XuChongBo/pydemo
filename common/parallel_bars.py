...
def progresser(n):
    ...
    for _ in tqdm(range(total), desc=text, position=n + 1, bar_format="{l_bar}%s{bar}%s{r_bar}" % (Fore.GREEN, Fore.RESET)):
        sleep(interval)
...
if __name__ == '__main__':
    ...
    for _ in trange(10, desc="1", position=1, bar_format="{l_bar}%s{bar}%s{r_bar}" % (Fore.BLUE, Fore.RESET)):
